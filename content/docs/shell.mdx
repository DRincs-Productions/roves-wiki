...
wait timeouts, redraws queued or coalesced, dispatched redraws, WebView paints, and final window
presents. The counters are disabled by default and create no sampling timer or background thread.
For meaningful Roves/Chrome comparisons, keep the page, release build, window size, warm-up and
sample duration identical; do not treat results from a debug build as representative.
